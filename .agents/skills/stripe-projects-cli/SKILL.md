...
3. Provision a resource with `stripe projects add <provider>/<service>`. Do not guess the `stripe projects add` argument. Run `stripe projects catalog <provider> --json` or `stripe projects catalog <provider>` and copy the exact `<provider>/<service>` slug before you run `stripe projects add`. Example: `stripe projects add databaseco/postgres --name primary-db`. Use `--name <resource>` to control the local resource name used by future resource commands and environment variable prefixes. If you omit `--name`, the CLI uses the provider/service default for the local resource name. When a service config field looks like a name, the CLI uses the current project name as the default value when that satisfies the field schema. Use `--config '<json>'` when the service requires configuration. If the user already has the resource at that provider — anything already deployed — read `provider.existing_resource_linking` from `stripe projects catalog <provider> --json` and pass `--existing` to adopt it instead of creating a duplicate alongside it. That field is `supported`, `unsupported`, or `unknown`. Pass `--existing` on `supported`; also pass it on `unknown`, which means the provider published no capability list and only the server can answer, so the attempt is what tells you. Only `unsupported` means do not pass it. You are never shown the create-or-link choice, because it needs a terminal you do not have, so `--existing` is the only way you can link one; when the provider cannot adopt, `add` fails with `EXISTING_RESOURCE_LINK_UNSUPPORTED` and the resource has to stay outside Projects. When `add` warns that the provider could have adopted an existing resource, say so in your summary rather than dropping it — a duplicate the user has to clean up later is worse than a question now.
...
You CANNOT complete browser authentication alone. If a command exits with `BROWSER_AUTH_REQUIRED`, Run `stripe login --non-interactive --new-session` to print JSON with `browser_url`, `verification_code`, and `next_step`; present `browser_url` and `verification_code` to the user, then run the emitted `next_step` command to complete login before retrying. `--new-session` is required: without it the Stripe CLI prints "already logged in" and exits 0 without authenticating, and that exit 0 is not success. If the CLI rejects `--new-session` as an unknown flag (Stripe CLI older than 1.50.0), run the same command without that flag — on those versions it prints the same JSON handoff when no session exists. If any login attempt prints "already logged in" instead of JSON, stop and tell the user to run `stripe projects init` themselves in a terminal with browser access — a session already exists, so `stripe login` prints the same "already logged in" for them and cannot authenticate Projects either. Never retry the original command until a login has actually completed. Try the login once only: if the same check still fails after a login that completed successfully, another sign-in will not change it — stop and tell the user to run `stripe projects init` themselves in a terminal with browser access. If a command exits with `PROJECTS_SESSION_UNUSABLE`, A Stripe CLI session exists for this account, but Stripe Projects cannot read live-mode credentials from it. A person has to run `stripe projects init` in a terminal with browser access to finish authenticating Projects. Nothing you can run resolves this, so do not retry the command. That hand-over holds only while nothing has signed in yet: read the message first, because if it says a sign-in already completed, then `stripe projects init` is the command that produced this and handing it back is the loop — report it and stop instead. If a command exits with `ACCOUNT_NOT_ELIGIBLE`, Three different failures share this code and `details.reason` says which, so branch on it rather than on the other fields: two of the three carry an `onboarding_url` and it does not mean the same thing in both. `account_not_onboarded`: the account was never onboarded and `details.onboarding_url` is the page that onboards it — hand it to the user, ask them to confirm they finished it for the account the message names, and retry only then. `eligibility_check_unanswered`: Stripe never answered the eligibility check and nothing refused anything, so retry once (`details.retryable` is true) before reporting anything. `project_creation_forbidden`: eligibility passed and Stripe refused to create the project anyway, so the same `onboarding_url` is only somewhere for the user to check that setup is finished — the other candidate is a credential that is not permitted to create projects, which needs a different key or role, so report both candidates and do not retry either. Never run `stripe projects switch-account` yourself for any of them: it needs a TTY you do not have, so it is something to ask the user for — unlike the codes above it, this one is not a single failure, and a browser step you cannot drive is only one of the three. If a command exits with `MERCHANT_MISMATCH`, `stripe projects switch-account` needs an interactive terminal, so you cannot switch accounts yourself. Report this to the user and ask them to run `stripe projects switch-account` in a terminal with browser access. Do not retry the command. If a command exits with `INVALID_API_KEY`, the STRIPE_API_KEY environment variable is set and unusable: either it is a key for the other mode (pass or drop `--test`, or change the key) or it could not authenticate (unset it or provide a valid key). This one is yours to fix and it is not a session problem, so read the message, fix the variable or the mode, and retry — do not run `stripe projects init` for it. If a command exits with `PROJECTS_CONTEXT_MISMATCH`, The Stripe CLI is pointed at a different mode than the one Projects needs. Read the message: it either names a `stripe switch context` command to run, or says the mode you asked for is unsupported and no command fixes it. You are already signed in either way, so do not run `stripe login` or `stripe projects init` for it — this is not a browser-auth stop, so it is not one of the codes above that a login can clear. If a command exits with `PROJECTS_HOST_AUTH_REJECTED`, Stripe refused the Stripe CLI's own sign-in, which is the one Projects uses on this machine. Report it and stop: ask the user to run `stripe login` in a terminal with browser access, or `stripe switch context <account> --live` if they meant a different account. Do not run `stripe projects init` for it — a Projects login adds a second credential that outranks the refused one rather than repairing it — this is not a browser-auth stop you can drive either, because the sign-in Stripe refused is the Stripe CLI's own rather than a Projects one. If a command exits with `PROJECTS_CREDENTIAL_STORE_UNREADABLE`, This machine's credential store could not be read, so Projects cannot tell which credential signs. Read the message for what the store said, report it to the user, and ask them to make the store readable — unlocking the system keyring is the usual fix. It is not a sign-in problem: do not run `stripe login` or `stripe projects init`, which write to the same store. Retry only once the user says the store is readable — a session may well exist, so this is not a browser-auth stop at all. If a command exits with `PROJECTS_ACCOUNT_IDENTITY_UNCONFIRMED`, Stripe could not be reached to confirm which account the stored credentials belong to, and the account named in local configuration is not written beside those credentials — so which account the command would act as is unknown. It is a connectivity failure, not a sign-in one: do not run `stripe login` or `stripe projects init`, which would overwrite working credentials. Retry once Stripe is reachable — alone among the codes above, this one is worth retrying unchanged, and it is the sign-in that must not be retried. If a command exits with `PROJECTS_ACCOUNT_READ_FORBIDDEN`, Stripe accepted the stored credentials and then refused them the account read, so which account the command would act as cannot be confirmed — a restricted key with Projects permission but no account-retrieve permission. It is neither a connectivity failure nor a sign-in one, so do not retry it unchanged: report it and ask the user either to set STRIPE_API_KEY to a key for this account that can retrieve the account, or to run `stripe projects init` in a terminal with browser access — it names the same missing answer as the code before it and takes the opposite instruction, so read which of the two you got before deciding to retry. Do NOT retry the original command until the blocker is resolved.
...
| `PROJECTS_SESSION_UNUSABLE` | Stripe CLI session exists but Projects cannot get live-mode credentials Stripe accepts from it — either none could be read, or a login completed and the credentials it produced were refused | Report the message to the user and stop. Do NOT retry, and do NOT run a login: the message says whether one has already been tried |
| `PROJECTS_CREDENTIAL_STORE_UNREADABLE` | This machine's credential store could not be read, so no credential can be chosen — a storage fault, not a missing session | This machine's credential store could not be read, so Projects cannot tell which credential signs. Read the message for what the store said, report it to the user, and ask them to make the store readable — unlocking the system keyring is the usual fix. It is not a sign-in problem: do not run `stripe login` or `stripe projects init`, which write to the same store. Retry only once the user says the store is readable |
| `PROJECTS_ACCOUNT_IDENTITY_UNCONFIRMED` | Stripe could not be reached to confirm which account the stored credentials act as, so no account can be bound to the work — a connectivity failure, not a sign-in one | Stripe could not be reached to confirm which account the stored credentials belong to, and the account named in local configuration is not written beside those credentials — so which account the command would act as is unknown. It is a connectivity failure, not a sign-in one: do not run `stripe login` or `stripe projects init`, which would overwrite working credentials. Retry once Stripe is reachable |
| `PROJECTS_ACCOUNT_READ_FORBIDDEN` | Stripe accepted the stored credentials and refused them the account read, so no account can be bound to the work — a missing key permission, not a connectivity or sign-in failure | Stripe accepted the stored credentials and then refused them the account read, so which account the command would act as cannot be confirmed — a restricted key with Projects permission but no account-retrieve permission. It is neither a connectivity failure nor a sign-in one, so do not retry it unchanged: report it and ask the user either to set STRIPE_API_KEY to a key for this account that can retrieve the account, or to run `stripe projects init` in a terminal with browser access |
| `BROWSER_AUTH_TIMEOUT` | Browser auth did not complete in time | Ask the user to finish the browser flow, then retry |
| `ACCOUNT_NOT_ELIGIBLE` | Account not onboarded for Projects, project creation refused, or the eligibility check went unanswered | Three different failures share this code and `details.reason` says which, so branch on it rather than on the other fields: two of the three carry an `onboarding_url` and it does not mean the same thing in both. `account_not_onboarded`: the account was never onboarded and `details.onboarding_url` is the page that onboards it — hand it to the user, ask them to confirm they finished it for the account the message names, and retry only then. `eligibility_check_unanswered`: Stripe never answered the eligibility check and nothing refused anything, so retry once (`details.retryable` is true) before reporting anything. `project_creation_forbidden`: eligibility passed and Stripe refused to create the project anyway, so the same `onboarding_url` is only somewhere for the user to check that setup is finished — the other candidate is a credential that is not permitted to create projects, which needs a different key or role, so report both candidates and do not retry either. Never run `stripe projects switch-account` yourself for any of them: it needs a TTY you do not have, so it is something to ask the user for |
...
| `EXISTING_RESOURCE_LINK_UNSUPPORTED` | `--existing` was passed to a provider that cannot adopt existing resources | Do not retry with `--existing`. Either provision a new resource without the flag, or leave the user's existing resource outside Projects and carry its credentials with `stripe projects variables set` |
...
| `PROJECTS_CONTEXT_MISMATCH` | The Stripe CLI's active context is in a different mode than Projects needs | The Stripe CLI is pointed at a different mode than the one Projects needs. Read the message: it either names a `stripe switch context` command to run, or says the mode you asked for is unsupported and no command fixes it. You are already signed in either way, so do not run `stripe login` or `stripe projects init` for it |
| `PROJECTS_HOST_AUTH_REJECTED` | Stripe refused the Stripe CLI's own credentials, which is the sign-in Projects uses on this machine | Stripe refused the Stripe CLI's own sign-in, which is the one Projects uses on this machine. Report it and stop: ask the user to run `stripe login` in a terminal with browser access, or `stripe switch context <account> --live` if they meant a different account. Do not run `stripe projects init` for it — a Projects login adds a second credential that outranks the refused one rather than repairing it |
...
- `stripe projects add databaseco/postgres --name primary-db` — example add command you can copy and adapt
    ...
    - `--existing` — link a resource the user already has at that provider instead of creating a new one
...
- `stripe projects remove <resource>` — deprovision a resource at the provider and stop tracking it; anything stored in it is deleted
    - `--only-credentials` — unlink the resource's credentials instead of deprovisioning it
    - `--untrack` — drop the local record without contacting the provider, leaving the provider resource in place and still billing. Use this when `remove` fails because the resource never finished provisioning or was already deleted at the provider; retrying plain `remove` will keep failing. Do not use it for a removal that failed for any other reason — the resource is probably still alive, so retry instead. It is one-way: no command re-tracks the resource here, and adding the service again provisions a second one beside it. Account plans are the exception, and are re-adopted the next time `add` or `update` needs one. `stripe projects status` lists the IDs it is hiding, under `untracked_resource_ids` in `--json`
...
<!-- stripe-projects-cli managed:1b6337f41198 -->
